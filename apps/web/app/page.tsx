        ...
        <div className="progress-track">
          <div
            ...
            style={{
              ...
              boxShadow: estourada ? "0 0 12px var(--danger-subtle)" : "0 0 16px rgba(52, 211, 153, 0.5)",
            }}
          />
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card p-6 animate-fade-in-up-delay-1">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-heading text-lg font-light text-primary flex items-center gap-2">
              <Clock className="w-4 h-4 text-accent" />
              Agenda Hoje ({eventosHoje.length})
            </h2>
            <Link href="/agenda" className="text-xs font-medium text-muted hover:text-accent flex items-center gap-1 uppercase tracking-wider">
          ...
          <div className="space-y-3">
            {eventosHoje.map((evento) => {
              ...
              return (
                <div
                  key={evento.id}
                  className="flex gap-3 p-3.5 rounded-2xl bg-surface hover:bg-surface-hover transition-colors border-l-4"
        ...
        <div className="card p-6 animate-fade-in-up-delay-2">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link href="/tarefas" className="text-xs font-medium text-muted hover:text-accent flex items-center gap-1 uppercase tracking-wider">
              Ver tudo <ChevronRight className="w-3 h-3" />
            </Link>
          </div>
          <div className="space-y-2.5">
            {tarefasHoje.map((t) => (
              <div
                ...
                className="flex items-center gap-3 py-2.5 px-3.5 rounded-2xl bg-surface hover:bg-surface-hover border border-border/40 transition-colors cursor-pointer"
              >
                <Circle
            ...
            {tarefasConcluidasHoje.map((t) => (
              <div
                key={t.id}
                onClick={() => toggleTarefa(t.id, t.status)}
                className="flex items-center gap-3 py-2 px-3.5 rounded-2xl opacity-40 cursor-pointer hover:opacity-75 transition-opacity"
              >
        ...
        <div className="card p-6 animate-fade-in-up-delay-3">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link href="/pipeline" className="text-xs font-medium text-muted hover:text-accent flex items-center gap-1 uppercase tracking-wider">
              Ver tudo <ChevronRight className="w-3 h-3" />
            </Link>
          </div>
          <div className="space-y-3">
            {videosAtivos.slice(0, 4).map((v) => {
              ...
              return (
                <Link
                  key={v.id}
                  href={`/pipeline/${v.id}`}
                  className="block p-3.5 rounded-2xl bg-surface hover:bg-surface-hover border-l-4 border-border transition-all"
                  style={{
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <p
        className="font-heading text-4xl font-light tracking-tight mt-1"
